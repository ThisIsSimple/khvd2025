	...
	>
		...
		<div class="relative flex items-start justify-between w-full">
			...
			<div class="flex gap-[12px] tablet:gap-[20px] xl:gap-[24px] items-start pt-[12px]">
				...
				<div>
					<p
						class="font-display text-[150px] desktop:text-[180px] xl:text-[210px] leading-[1.2] transition-colors duration-500 {isHovered
							? 'text-[#fefefe]'
							: 'text-[#111111]'}"
					>
						{number}
				...
				{#if isHovered}
					...
					>
						<div
							use:motion
							class="flex items-start h-full py-[28px] desktop:py-[34px] xl:py-[40px] gap-[8px] overflow-hidden pr-[40px]"
						>
							{#if category}
								<div class="flex flex-col font-display text-[40px] leading-none text-[#fefefe]">
									GRADUATION<br />STUDIES
								</div>
							{/if}

							<p class="font-display text-[40px] leading-none text-[#fefefe] whitespace-nowrap">
								[{workCount}]
							</p>
						</div>
					</Motion>
				...
				{#if !isHovered}
					<div class="flex items-center h-full py-[28px] desktop:py-[34px] xl:py-[40px]">
						<p class="font-display text-[40px] leading-none text-[#111111]">
							[{workCount}]
						</p>
					</div>
				{/if}
			...
			{#if isHovered}
				<img
					...
					alt=""
					class="w-[80px] h-[80px] absolute top-[40px] desktop:top-[48px] xl:top-[52px] right-[40px] desktop:right-[48px] xl:right-[52px]"
				/>
			{/if}
		</div>
		...
		{#if isHovered && title}
			...
			>
				<div use:motion class="flex flex-col text-[#fefefe]">
					{#if category}
						<p class="font-display text-[40px] uppercase whitespace-nowrap">
							{category}
						</p>
					{/if}
					<p class="font-semibold text-[40px] leading-[1.1] tracking-[-2.4px] whitespace-pre-line">
						{title}
					</p>
				</div>
			</Motion>

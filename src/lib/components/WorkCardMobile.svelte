...
>
	<div
		use:motion
		class="relative flex items-end justify-between px-[16px] w-full transition-all duration-500 ease-in-out"
		onmouseenter={() => (isHovered = true)}
		onmouseleave={() => (isHovered = false)}
		ontouchstart={() => (isHovered = !isHovered)}
		role="button"
		tabindex="0"
	>
		<!-- Background Pattern -->
		...
		<div class="relative flex flex-col gap-[4px] items-start">
			...
			{#if isHovered && (category || title)}
				<Motion
					...
					let:motion
				>
					<div use:motion class="flex flex-col gap-[8px] overflow-hidden">
						{#if category}
						...
						{/if}
						{#if title}
							<p class="font-semibold text-[28px] leading-[1.3] text-[#111111] max-w-[200px]">
								{title}
							</p>
						{/if}
					</div>
